import os
import subprocess

common_prefix = "post-MIRU2025"
common_suffix = "point_label_calibrated_batch12"
common_args = "--batch_size 12"
common_platform = ""


dict_cfg_lyft2kitti_default = {
    "name": f"{common_prefix}_lyft2kitti_{common_suffix}",
    "script": 'train.py',
    "cfg_file": "cfgs/da-post-MIRU2025/second_old_anchor_basebev_multi_lyft2kitti_car_ped_point_label_calibrated.yaml",
    "teacher_ckpt": "",
    "pretrained_ckpt": "",
    "ckpt_dir": "",
    "args": common_args,
    "platform": common_platform,
}

dict_cfg_lyft2kitti_target = {
    "name": f"{common_prefix}_target_lyft2kitti_io1npo_{common_suffix}",
    "script": 'train.py',
    "cfg_file": "cfgs/da-post-MIRU2025/second_old_anchor_st3d_basebev_multi_lyft2kitti_target_car_ped_point_label_calibrated.yaml",
    "teacher_ckpt": "/storage/wandb/run-20250303_153637-io1npo3n/files/ckpt/checkpoint_epoch_50.pth",
    "pretrained_ckpt": "/storage/wandb/run-20250303_153637-io1npo3n/files/ckpt/checkpoint_epoch_50.pth",
    "ckpt_dir": "",
    "args": common_args,
    "platform": common_platform,
}

dict_cfg_lyft2kitti_dann_source = {
    "name": f"{common_prefix}_dann_lyft2kitti_io1npo3n_{common_suffix}",
    "script": 'train.py',
    "cfg_file": "cfgs/da-post-MIRU2025/second_old_anchor_st3d_basebev_multi_lyft2kitti_dann_source_car_ped_point_label_calibrated.yaml",
    "teacher_ckpt": "/storage/wandb/run-20250303_153637-io1npo3n/files/ckpt/checkpoint_epoch_50.pth",
    "pretrained_ckpt": "/storage/wandb/run-20250303_153637-io1npo3n/files/ckpt/checkpoint_epoch_50.pth",
    "ckpt_dir": "",
    "args": common_args,
    "platform": common_platform,
}


dict_cfgs = [
    dict_cfg_lyft2kitti_default,
    # dict_cfg_lyft2kitti_target,
    # dict_cfg_lyft2kitti_dann_source
]

for dict_cfg in dict_cfgs:
    cmd = "python3"
    if dict_cfg["script"]:
        cmd += " " + dict_cfg["script"]
    if dict_cfg["cfg_file"]:
        cmd += " --cfg_file " + dict_cfg["cfg_file"]
    if dict_cfg["name"]:
        cmd += " --run_name " + dict_cfg["name"]
    if dict_cfg["args"]:
        cmd += " " + dict_cfg["args"]
    if dict_cfg["platform"]:
        cmd += " " + dict_cfg["platform"]
    if dict_cfg["teacher_ckpt"]:
        cmd += " --pretrained_model_teacher " + dict_cfg["teacher_ckpt"]
    if dict_cfg["pretrained_ckpt"]:
        cmd += " --pretrained_model " + dict_cfg["pretrained_ckpt"]
    if dict_cfg["ckpt_dir"]:
        cmd += " --ckpt_dir " + dict_cfg["ckpt_dir"] + " --eval_all"

    print(cmd)
    subprocess.call(cmd.split())
