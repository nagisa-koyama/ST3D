...
RUN echo 'alias train_dev="bash scripts/dist_train.sh 2 --cfg_file cfgs/da-waymo-lyft-pandaset-to-kitti_models/domain_attention_head_per_dataset/second_old_anchor_kitti_waymo_lyft_pandaset_to_kitti.yaml --batch_size 40 --sync_bn"' >> /root/.bashrcUN echo 'alias train_dev_wo_kitti="bash scripts/dist_train.sh 2 --cfg_file cfgs/da-waymo-lyft-pandaset-to-kitti_models/domain_attention_head_per_dataset/second_old_anchor_waymo_lyft_pandaset_to_kitti.yaml --batch_size 40 --sync_bn"' >> /root/.bashrc
...
RUN echo 'alias test_all_dev_single2="python test.py --ckpt_dir /storage/wandb/run-20230821_222115-469by94p/files/ --eval_all --cfg_file cfgs/da-waymo-lyft-pandaset-to-kitti_models/domain_attention_head_per_dataset/second_old_anchor_waymo_lyft_pandaset_to_kitti.yaml"' >> /root/.bashrc
