...
"""
    ...
    Ported (near-verbatim) from UADA3D's pcdet/models/discriminators/discriminator.py
    to enable a like-for-like A/B comparison against ST3D's own domain-adaptation methods.
"""
...
class Discriminator(nn.Module):
    """Marginal-only discriminator (used by UADA3D's DA_IASSD variant). Kept for config-name
    compatibility with the shared discriminators.__all__ registry; not exercised by
    second-rospm-C / centerpoint-rospm-C (both use Discriminator2, conditional-only)."""
