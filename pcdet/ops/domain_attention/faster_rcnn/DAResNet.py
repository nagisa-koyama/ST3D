...
class DABasicBlock(nn.Module):
  ...
  def __init__(self, inplanes, planes, stride=1, downsample=None, fixed_block=False, num_datasets=None):
    ...
    self.domain_attention = DomainAttention(planes, reduction=16, nclass_list=None, fixed_block=fixed_block, num_adapters=num_datasets)
...
class DABottleneck(nn.Module):
  ...
  def __init__(self, inplanes, planes, stride=1, downsample=None, fixed_block=False, num_datasets=None):
    ...
    assert num_datasets is not None
    ...
    self.domain_attention = DomainAttention(planes * 4, reduction=16, nclass_list=None, fixed_block=fixed_block, num_adapters=num_datasets)
    self.relu = nn.ReLU(inplace=True)
    self.downsample = downsample
    self.stride = stride
